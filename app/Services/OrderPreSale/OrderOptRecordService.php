...
class OrderOptRecordService extends BaseService
{
    protected $theRepository;

    public function __construct() {
        $this->theRepository = new OrderOptRecordRepository();
    ...
    public function getList($request=null) {
        ...
        if ($list[$responseDto::DTO_FIELD_LIST]) {
            ...
            foreach ($list[$responseDto::DTO_FIELD_LIST] as $key => $v_detail) {
                $v_info = new OrderOptRecordDto();
                $v_info->Assign($this->addOptTypeName($v_detail));
