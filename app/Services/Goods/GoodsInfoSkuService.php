...
class GoodsInfoSkuService extends BaseService
{
    protected $theRepository;
    protected $goodsInfoRepository;

    public function __construct() {
        $this->theRepository = new GoodsInfoSkuRepository();
        $this->goodsInfoRepository = new GoodsInfoRepository();
    ...
    public function getList() {
        ...
        if (!isset($request['status'])) {
            $request['status'] = 1; // 添加手工单 选商品，默认只显示有效的
        }
