...
use App\Dto\SysPrivilegeDto;
use App\Repositories\Admin\SysPrivilegeRepository;
use App\Repositories\Admin\UserRepository;
use App\Services\BaseService;
use Illuminate\Support\Facades\Validator;


class SysPrivilegeService extends BaseService
{
    protected $theRepository;

    public function __construct() {
        $this->theRepository = new SysPrivilegeRepository();
    ...
    public function getList() {
        ...
        if (!isset($request['limit'])) $request['limit'] = 10000; // 权限默认返回全部
        if (!isset($request['status'])) $request['status'] = 1;   // 默认只返回使用状态的数据
        ...
        if ($list[$responseDto::DTO_FIELD_LIST]) {
            ...
            $list[$responseDto::DTO_FIELD_LIST] = $this->getPrivilegeTree($list[$responseDto::DTO_FIELD_LIST]);
        }
    ...
    // 获取树形结构，该方法时间复杂度高，有空再优化.
    public function getPrivilegeTree($data, $pid = 0, $level = 1) {
        $arr = [];
        foreach ($data as $v) {
            if ($v['parent_id'] == $pid) {
                $v['level'] = $level;
                $v['children'] = $this->getPrivilegeTree($data, $v['id'], $level + 1);
                $v_info = new SysPrivilegeDto();
                $v_info->Assign($v);
                $arr[] = $v_info;
            }
        }
