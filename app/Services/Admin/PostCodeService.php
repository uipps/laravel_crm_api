...
use App\Repositories\Admin\PostCodeRepository;
...
class PostCodeService extends BaseService
{
    protected $theRepository;

    public function __construct() {
        $this->theRepository = new PostCodeRepository();
    ...
    public function getPostCodeByDistrictId($id) {
        $request['id'] = $id;
        $api = 'load_post_code.html?district_id=';
        $responseDto = new ResponseDto();
        ...
        // 获取数据，包含总数字段 TODO 以后改用本地数据库
        // 直接请求接口
        $url = env('AREA_API_DOMAIN', 'http://frontweb.esmtong.cn') . '/index/area/' . $api . $request['id'];
        /*$client = new \GuzzleHttp\Client();
        $res = $client->request('GET', $url);
        // 或者
        $client = new Client([
            'base_uri' => 'http://frontweb.esmtong.cn', // 根域名
            'timeout'  => 5.0,  // 超时
        ]);
        $res = $client->get($url);
        $list = $res->getBody()->getContents();*/
        $list = @file_get_contents($url);
        $list = json_decode($list, true); // 转成数组
        ...
        $list['page'] = 1;
        $list['limit'] = $list['total'] > 0 ? $list['total'] : 20;
