...
use App\Dto\CustomerClueTrackDto;
...
class CustomerClueTrackService extends BaseService
{
    protected $theRepository;
    protected $customerClueRepository;

    public function __construct() {
        $this->theRepository = new CustomerClueTrackRepository();
        $this->customerClueRepository = new CustomerClueRepository();
    ...
    public function getList($request=null) {
        ...
        if ($list[$responseDto::DTO_FIELD_LIST]) {
            ...
            foreach ($list[$responseDto::DTO_FIELD_LIST] as $key => $v_detail) {
                $v_info = new CustomerClueTrackDto();
                $v_info->Assign($this->addCreatorName($v_detail));
    ...
    public function addOrUpdate($request = null) {
        ...
        $rules = [
            ...
            'clue_id' => 'required|integer|min:1',
        ...
        if (isset($request['id']) && $request['id']) {
            ...
            // 该条记录对应的线索clue_id需要变更为已处理
            $clue_info = $this->customerClueRepository->getInfoById($data_arr['clue_id']);
            if ($current_uid == $clue_info['post_sale_id'] && 1 != $clue_info['opt_status']) {
                // 已经分配给该登录用户，未处理的变为已处理
                $this->customerClueRepository->updateData($data_arr['clue_id'], ['opt_status' => 1]);
            }
            try {
                // TODO 加事务，现在追踪记录数要加1
                CustomerClue::find($data_arr['clue_id'])->increment('track_num');  // 原子+1
            } catch(\Exception $e){
                \Log::error('clue-increment-track_num-error: ' . $e->getMessage());
            }
