...
use App\Dto\CustomerRemarkDto;
use App\Repositories\Customer\CustomerRemarkRepository;
...
class CustomerRemarkService extends BaseService
{
    ...
    public function __construct() {
        $this->theRepository = new CustomerRemarkRepository();
    ...
    public function getList() {
        ...
        if ($list[$responseDto::DTO_FIELD_LIST]) {
            // 创建时间倒序，对二维数组进行排序
            $order_sequence = 'desc'; // asc:升序，从小到大, desc:倒序
            if ($list[$responseDto::DTO_FIELD_LIST]) {
                global $key2sort;
                $key2sort = 'id'; // 排序字段
                if ('asc' == $order_sequence) {
                    // 升序，从小到大
                    usort($list[$responseDto::DTO_FIELD_LIST], 'self::compare');
                } else {
                    usort($list[$responseDto::DTO_FIELD_LIST], 'self::compare_r');
                    //uasort($list[$responseDto::DTO_FIELD_LIST], 'self::compare_r'); 不需要索引，保证列表是数组
                }
            }

            // 成功，返回列表信息
            foreach ($list[$responseDto::DTO_FIELD_LIST] as $key => $v_detail) {
                $v_info = new CustomerRemarkDto();
                $v_info->Assign($this->addPreSaleIdName($v_detail,[],'real_name','creator_id','sale_name'));
    ...
    public function addOrUpdate($request = null) {
        ...
        if (isset($request['id']) && $request['id']) {
            ...
            if ($current_uid != $v_detail['creator_id']) {
                ErrorMsg::FillResponseAndLog($responseDto, ErrorMsg::CUSTOMER_REMARK_CANNOT_MODIFY_BY_OTHER);
                return $responseDto;
            }
    ...
    // 比较函数，按照正序排列
    public static function compare($arr1, $arr2) {
        global $key2sort;
        if ($arr1[$key2sort] > $arr2[$key2sort]) return 1;
        if ($arr1[$key2sort] < $arr2[$key2sort]) return -1;
        if ($arr1['id'] > $arr2['id']) return 1;
        if ($arr1['id'] < $arr2['id']) return -1;
        return -1;
    }
    // 倒序
    public static function compare_r($arr1, $arr2) {
        global $key2sort;
        if ($arr1[$key2sort] > $arr2[$key2sort]) return -1;
        if ($arr1[$key2sort] < $arr2[$key2sort]) return 1;
        if ($arr1['id'] > $arr2['id']) return -1;
