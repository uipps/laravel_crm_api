...
class UpdateRequest extends FormRequest
{
    ...
    {
        if ($this->role->status != CommonMapper::STATUS_SHOW) {
            $exception = new AuthorizationException('当前用户没有修改此角色信息的权限');
            $exception->detail = ['该角色处于无效状态，不可修改。'];
            throw $exception;
        }
        return true;
    }
    ...
    {
        return [
            'name' => 'sometimes|required|string|max:16',
            'remark' => 'sometimes|required|string|max:300',
        ];
    }
