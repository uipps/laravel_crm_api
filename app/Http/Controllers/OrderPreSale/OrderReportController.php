...
use App\Services\OrderPreSale\OrderReportService;
...
class OrderReportController extends CommonController
{
    ...
    public function list(Request $request) {
        ...
        if($request->input('request_action') == 'download'){
            ...
            // $opt_type_list = OrderOptType::pluck('name', 'id');
            // $opt_type_list[0] = '-';
            ...
            exportCsv($field_list, function()use($query){
                ...
                $opt_type_list = Arr::get($fieldMap,'order.pre_opt_type.enum');
                // $auditStatus = Arr::get($fieldMap,'order.audit_status.enum');
                $orderStatus = Arr::get($fieldMap,'order.order_status.enum');
                $ordeType = Arr::get($fieldMap,'order.order_type.enum');

                $limit = 1000;
                $ret = $query->orderByDesc('pre_opt_time')->paginateFilter($limit);
                ...
                foreach($ret as &$item){
                    ...
                    $item['pre_opt_type'] = Arr::get($opt_type_list, $item['pre_opt_type']);

                    $item['order_status'] = Arr::get($orderStatus, $item['order_status']);
                    $item['order_type'] = Arr::get($ordeType, $item['order_type']);
            ...
            $ret = $query->orderByDesc('pre_opt_time')->paginateFilter($request->input('limit'));
            $ret = OrderReportResource::collection($ret)->resource;
