...
class RolePrivilegeRepositoryImpl extends BaseRepository
{
    protected $model ;
    ...
    // 删除角色对应的权限
    public function deleteMultiPrivilegeByRoleid($role_id) {
        $sql_where = [
            'role_id' => $role_id,
        ];
    ...
    // 通过id获取信息
    public function getPrivilegesByRoleId($role_id) {
        $db_result = $this->model->where('role_id', $role_id)->get()->toArray();
        if ($db_result)
            return array_column($db_result, 'privilege_id');
        return $db_result;
    }
