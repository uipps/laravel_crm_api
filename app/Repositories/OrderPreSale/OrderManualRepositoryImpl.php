...
class OrderManualRepositoryImpl extends BaseRepository
{
    protected $model ;
    protected $orderModel ;

    public function __construct() {
        $this->model = new OrderManual();
    ...
    public function getReplenishBySourceOrderId($source_order_id) {
        $sql_where = [
            'source_order_id' => $source_order_id,
            'type' => OrderManual::TYPE_REPLENISH,
        ];
        $db_result = $this->model->where($sql_where)->get()->toArray();
        return $db_result;
    }

    public function getReDeliveryBySourceOrderId($source_order_id) {
        $sql_where = [
            'source_order_id' => $source_order_id,
            'type' => OrderManual::TYPE_REDELIVERY
        ];
        $db_result = $this->model->where($sql_where)->get()->toArray();
        return $db_result;
    }
