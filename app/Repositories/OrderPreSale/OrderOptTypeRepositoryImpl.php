...
class OrderOptTypeRepositoryImpl extends BaseRepository
{
    protected $model ;

    public function __construct() {
        $this->model = new OrderOptType();
    }
    ...
    public function getPreOptTypeList() {
        $db_result = $this->model->whereBetween('id', [11, 24])->get()->toArray();
        if ($db_result)
            return array_column($db_result, null, 'id');
        return $db_result;
    }

    public function getReportPreOptTypeList() {
        $db_result = $this->model->whereBetween('id', [10, 24])->get()->toArray();
        if ($db_result)
            return array_column($db_result, null, 'id');
        return $db_result;
    }
