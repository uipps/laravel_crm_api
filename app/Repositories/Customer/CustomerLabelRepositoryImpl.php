...
class CustomerLabelRepositoryImpl extends BaseRepository
{
    protected $model ;

    public function __construct() {
        $this->model = new CustomerLabel();
    }
    ...
    public function getInfoByCustomerIdType($customer_id, $label_type) {
        ...
        // 只获取一条
        $db_result = $this->model->where($sql_where)->orderBy('id', 'DESC')->first();// 返回null，不能用->toArray()
        if (!$db_result)
            return $db_result;
        return $db_result->toArray();
    }
