...
use App\Models\Customer\CustomerClueTrack;
use App\Repositories\BaseRepository;

class CustomerClueTrackRepositoryImpl extends BaseRepository
{
    protected $model ;

    public function __construct() {
        $this->model = new CustomerClueTrack();
    }
    ...
    //
    public function insertTrackByClueId($clue_id, $request,  $created_time, $creator_id) {
        $data_arr = [
            'clue_id' => $clue_id,
        ...
        if (!isset($data_arr['status'])) $data_arr['status'] = 1; // 新增, 默认都是启用
